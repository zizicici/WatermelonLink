...
Generated Links are counted after Turnstile verification and ticket issuance, and successful connections are counted after the browser reports signaling completion. `/var/lib/watermelon-link/usage.json` stores UTC-hour aggregates and UTC-day summaries for up to 100 days, plus all-time totals that are never removed by the rolling window, with browser-family and operating-system-family breakdowns for both stages. Unique-network counts use a date-scoped HMAC of the canonical public IPv4 address or IPv6 `/64`; the raw address and full User-Agent are not persisted, so all-time totals intentionally do not claim a cross-day unique-network count. The file also contains no ticket data, file names, folder paths, or transfer details. Metrics run fail-open outside request awaits, coalesce writes behind at most one in-flight operation, flush at most once per minute, cap current-period deduplication state, discard old hash sets while retaining their aggregate counts, and reject aggregate files above 10 MiB. View the last 30 days and all-time totals with `node /opt/watermelon-link/current/scripts/usage-report.mjs`; pass a day count up to `100`, and add `--json` for machine-readable output without the internal HMAC values.
