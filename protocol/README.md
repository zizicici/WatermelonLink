...
The machine-readable client-to-server signaling envelope is defined in `v1/client-signaling.schema.json`. Server control messages, authenticated DataChannel JSON, and binary file frames are directional protocol messages documented below rather than members of that envelope.
...
- Room count, lifetime, signal messages, message size, cumulative signal bytes, WebSocket fragments and client control frames, per-network pre-verification WebSocket upgrades, raw global upgrades, globally accepted signed upgrades, server connections, and per-peer outbound WebSocket buffering all have hard limits. The per-network upgrade budget is consumed before the raw global budget so one network cannot drain the shared allowance with already-rate-limited requests. Automatic PONG is disabled; bounded PING replies and server heartbeats share the same outbound backpressure boundary, and control events yield between frames. Rejected upgrades are force-closed after a fixed deadline so a half-open peer cannot retain an application socket. Resource accounting is released before socket shutdown and does not depend on a peer emitting `close`.
...
The phone rejects a node-declared download above 64 GiB before creating a local file, caps repository SQLite downloads at 256 MiB and lock bodies at 1 MiB, and preserves 128 MiB of reported local capacity. Legacy resources without a content hash must match the manifest size exactly. Hashed resources use the SHA-256 of the stored bytes as authoritative because older manifests could contain PhotoKit's pre-export size rather than the exported file size. Download frames can never exceed the size accepted at `download_begin`.
